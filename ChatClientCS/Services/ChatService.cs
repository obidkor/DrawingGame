...
namespace ChatClientCS.Services
{
    ...
    {
        public event Action<string, MessageType> GameStart;
        public event Action<string, MessageType> GameEnd;
        public event Action<string, string, MessageType> SetNewHost;
        public event Action<string, string, MessageType> NewTextMessage;
        public event Action<string, byte[], MessageType> NewImageMessage;
        public event Action<string, byte[], MessageType> NewStrokesCollected;
        ...
        public event Action<string, string> Correct;
        ...
        public async Task BeginGameAsync()
        {
            await hubProxy.Invoke("BeginGame");
        }

        public async Task EndGameAsync()
        {
            await hubProxy.Invoke("EndGame");
        }
        ...
        public async Task SendBroadcastStrokesAsync(byte[] img)
        {
            await hubProxy.Invoke("BroadcastStrokes", img);
        }
        ...
        public async Task SendAnswerAsync(string msg)
        {
            await hubProxy.Invoke("IsRight", msg);
        }
